...
/* This is just for the enter key when it's pressed */

   /* var inputInfo = document.querySelector(".search-Tag");
    inputInfo.addEventListener("keyup", function(event) {
        event.preventDefault(); // Cancel the default if needed
        if (event.keyCode === 13)
            searchImages();
    }); */

$(".search-Tag").keypress((e) => {
    if(e.keyCode == 13) {
        searchImages();
        // The preventDefault avoids reloading the page after the result
        e.preventDefault();
    }
});
...
function removeImage(tag) {
    if (confirm('The image - '+ tag.trim() +' - will be removed.\n Do you confirm?')) {
        // var deleteForm  = document.querySelector("#deleteForm");

        // Getting the form
        var deleteForm = document.getElementById("deleteForm");
        var inputTag = document.getElementById("targetTag");
